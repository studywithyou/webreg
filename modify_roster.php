...
else
{
	...
	if ($release==1)
	{
		...
		$sth1 = $db->prepare('DELETE FROM teams WHERE id = ?');
		$sth2 = $db->prepare("UPDATE teams SET ibl_team = 'FA' WHERE id = ?");
		...
		foreach ($release_list as $release_id)
		{
			...
			$query = $db->query($sql);
			$query->fetchInto($row,DB_FETCHMODE_ASSOC);
			$status = $row["status"];
			
			if ($status == 3) { // player is uncarded, so they get deleted
				$db->execute($sth1, $release_id);
			} else {
				$db->execute($sth2, $release_id);
			}
	...
	if ($modify==1)
	{
		...
		$sth = $db->prepare("UPDATE teams SET tig_name = ?, item_type = ?, status = ?, comments = ? WHERE id = ?");
		...
		{			
					...
					|| $update_status!=$shadow_status[$modify_id])
			{
				...
				$db->execute($sth, array(stripslashes($update_tig_name), $update_type, $update_status, $update_comments, $modify_id));
			}
		}
	...
	if ($add==1)
	{
		...
		$sth = $db->prepare('INSERT INTO teams (tig_name, ibl_team, item_type, comments, status) VALUES (?, ?, ?, ?, ?');
		$db->execute($sth, array(stripslashes($new_tig_name), $ibl_team, $new_type, $new_comments, $new_status));
	}
